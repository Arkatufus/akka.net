...
public class ActorSelection : ICanTell
{
    ...
    /// <summary>
    /// A string representation of all the elements in the <see cref="ActorSelection"/> path,
        ...
        foreach (var s in elements)
        {
            switch (s)
            {
                ...
                case "**":
                    ...
                    break;
                case var _ when s.Contains('?') || s.Contains('*'):
                    list.Add(new SelectChildPattern(s));
                    break;
                case "..":
    ...
    {
        if (sender == null && ActorCell.Current != null && ActorCell.Current.Self != ActorRefs.NoSender)
            sender = ActorCell.Current.Self;

        DeliverSelection((IInternalActorRef)Anchor, sender,
    ...
    private async Task<IActorRef> InnerResolveOne(TimeSpan timeout, CancellationToken ct)
    {
        try
        {
            var identity = await this.Ask<ActorIdentity>(new Identify(null), timeout, ct);
                ...
                if (actorRef is ActorRefWithCell refWithCell)
                {
                    EmptyLocalActorRef EmptyRef(){
                        return new EmptyLocalActorRef(
                            ...
                            path: anchor.Path / sel.Elements.Select(el => el.ToString() ?? string.Empty),
                    ...
                    switch (iter.Next())
                    {
                        ...
                        case SelectChildRecursive _:
                            ...
                            var msg = new ActorSelectionMessage(sel.Message, [SelectChildRecursive.Instance], true);
                            foreach (var c in allChildren)
                            {
                                c.Tell(sel.Message, sender);
                                DeliverSelection((IInternalActorRef)c, sender, msg);
                            }
                            break;
                        case SelectChildPattern pattern:
                            ...
                            if (iter.IsEmpty())
                            {
                                ...
                                {
                                    foreach (var actor in matchingChildren)
                                        actor.Tell(sel.Message, sender);
                                }
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    foreach (var actor in matchingChildren)
                                        DeliverSelection((IInternalActorRef)actor, sender, message);
                                }
                            }
                            break;
                    }
    ...
    public override bool Equals(object? obj)
    {
        ...
        return obj is ActorSelection other && Equals(other);
    }
...
public class SelectChildName : SelectionPathElement
{
    ...
    {
        if (ReferenceEquals(this, obj)) return true;
        return obj is SelectChildName other && Equals(other);
    }
...
public class SelectChildPattern : SelectionPathElement
{
    ...
    {
        if (ReferenceEquals(this, obj)) return true;
        return obj is SelectChildPattern other && Equals(other);
    }
...
public class SelectChildRecursive : SelectionPathElement
{
    ...
    {
        if (ReferenceEquals(this, obj)) return true;
        return obj is SelectChildRecursive other && Equals(other);
    }
...
public class SelectParent : SelectionPathElement
{
    ...
    public override bool Equals(object? obj) => obj is SelectParent;
