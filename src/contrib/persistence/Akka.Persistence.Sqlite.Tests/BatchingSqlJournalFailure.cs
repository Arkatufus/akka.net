...
akka {{
    ...
    persistence {{
        ...
        public async Task Bug4265_Persistent_actor_stuck_with_RecoveryTimedOutException_after_circuit_breaker_opens()
        {
            ...
            Watch(actor);
            _probe.ExpectMsg("ReplaySuccess", timeout);
            _probe.ExpectMsg<RecoveryCompleted>(timeout);
    ...
    internal class JournalFailForXTimes : IJournalInterceptor
    {
        private readonly int _totalFails;
        private int _failCount;

        public JournalFailForXTimes(int totalFails)
        {
    ...
    internal class SnapshotFailForXTimes : ISnapshotStoreInterceptor
    {
        private readonly int _totalFails;
        private int _failCount;

        public SnapshotFailForXTimes(int totalFails)
        {
            _totalFails = totalFails;
            _failCount = 0;
        }
