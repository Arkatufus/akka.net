...
namespace Akka.Persistence.Sqlite.Tests.Internal.SnapshotStore
{
    /// <summary>
    ///     Sqlite persistence snapshot store implementation which behavior could be controlled by interceptors.
    /// </summary>
    public class TestSqliteSnapshotStore : SqliteSnapshotStore
    {
        ...
        public TestSqliteSnapshotStore(Config snapshotConfig) : base(snapshotConfig)
        {
        }
        ...
        /// </summary>
        /// <remarks>
        ///     Snapshot store actor must be of <see cref="TestSqliteSnapshotStore"/> type.
        /// </remarks>
        /// <param name="actor">Journal actor reference.</param>
        /// <returns>Proxy object to control <see cref="TestSqliteSnapshotStore"/>.</returns>
        public static ITestSnapshotStore FromRef(IActorRef actor)
        {
            return new TestSqliteSnapshotStoreWrapper(actor);
        }
        ...
        internal class TestSqliteSnapshotStoreWrapper : ITestSnapshotStore
        {
            public TestSqliteSnapshotStoreWrapper(IActorRef actor)
            {
                _actor = actor;
            }
